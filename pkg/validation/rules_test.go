...
func TestEach_EmptyAndNil(t *testing.T) {
	t.Parallel()

	fail := func(string) error { return errors.New("boom") }

	require.Empty(t, validation.Each("tags", []string{}, fail)())
	require.Empty(t, validation.Each[[]string]("tags", nil, fail)())
}

func TestEach_StampsIndexedField(t *testing.T) {
	t.Parallel()

	// Fail only on the empty element so we can pin the index.
	...
	rule := validation.Each("tags", []string{"ok", "", "", "fine"}, check)

	errs := rule()
	require.Len(t, errs, 2)
	require.Equal(t, "tags[1]", errs[0].Field)
	require.Equal(t, "tags[2]", errs[1].Field)
	require.Equal(t, "is required", errs[0].Message)
}

func TestEach_NormalizesCheckOutput(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{
			name:     "plain error is wrapped with indexed Field name",
			checkRet: errors.New("boom"),
			want:     validation.Errors{{Field: "tags[0]", Message: "boom"}},
		},
		{
			name:     "validation.Error with empty Field gets stamped",
			checkRet: validation.Error{Message: "boom"},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			check := func(string) error { return tt.checkRet }
			rule := validation.Each("tags", []string{"foo"}, check)
			require.Equal(t, tt.want, rule())
		})
	}
}

func TestEach_RunsAllChecks(t *testing.T) {
	t.Parallel()

	failA := func(string) error { return errors.New("a") }
	failB := func(string) error { return errors.New("b") }
	rule := validation.Each("tags", []string{"x", "y"}, failA, failB)

	errs := rule()
	require.Len(t, errs, 4)
	require.Equal(t, "tags[0]", errs[0].Field)
	require.Equal(t, "a", errs[0].Message)
	require.Equal(t, "tags[0]", errs[1].Field)
	require.Equal(t, "b", errs[1].Message)
	require.Equal(t, "tags[1]", errs[2].Field)
	require.Equal(t, "tags[1]", errs[3].Field)
}

func TestEach_DoesNotMutateCheckOutput(t *testing.T) {
	t.Parallel()

	// A check that returns the same validation.Errors value every time. If
	// Each mutates the returned slice in place, the second call will see the
	// entries' Field already populated and silently skip stamping.
	shared := validation.Errors{{Message: "boom"}}
	check := func(string) error { return shared }

	first := validation.Each("first", []string{"x"}, check)()
	second := validation.Each("second", []string{"x"}, check)()

	require.Len(t, first, 1)
	require.Len(t, second, 1)
	require.Equal(t, "first[0]", first[0].Field)
	require.Equal(t, "second[0]", second[0].Field, "second Each call must stamp independently")
	require.Empty(t, shared[0].Field, "the caller's Errors value must not be mutated")
}
